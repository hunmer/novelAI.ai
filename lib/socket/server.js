...
module.exports = (io) => {
  io.on('connection', async (socket) => {
    ...
    // await logger.info(`WebSocket 客户端连接`, 'socket', { socketId: socket.id });
    ...
    socket.on('join-project', async (data) => {
      ...
      // await logger.info(`用户加入项目房间`, 'socket', {
      //   projectId,
      //   userId,
      //   userName,
      //   socketId: socket.id,
    ...
    socket.on('project:update', async (data) => {
      ...
      await logger.debug(`项目更新同步`, 'socket', {
        projectId,
        module,
        version,
        userId,
    ...
    socket.on('disconnect', async () => {
      ...
      // await logger.info(`WebSocket 客户端断开连接`, 'socket', { socketId: socket.id });

      // 清理在线用户记录
      onlineUsers.forEach(async (users, projectId) => {
        if (users.has(socket.id)) {
          ...
          // await logger.info(`用户断开连接`, 'socket', {
          //   projectId,
